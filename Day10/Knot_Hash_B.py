...
# 	-	Then, the current position moves forward by the length, 3, plus the skip size, 0: 2 1 0 [3] 4. Finally, the
...
# Functions
#######################################################################################################################
def hash_it(commands, knot_list):
    knot_list_copy = knot_list.copy()
    commands_tail = [17, 31, 73, 47, 23]
    ...
    # Sparse hash
    for i in range(64):
        knot_results = knot(commands_full, knot_list_copy, current_index, skip_size)
        knot_list_copy = knot_results.get("Knot list")
        current_index = knot_results.get("Index")
        skip_size = knot_results.get("Skip size")
...
def knot(commands, knot_list, current_index, skip_size):
    for command in commands:
        knot_list = knot_execute(command, current_index, knot_list)
        current_index = (current_index + command + skip_size) % len(knot_list)
        skip_size += 1

    return {"Knot list": knot_list, "Index": current_index, "Skip size": skip_size}


def knot_execute(command, start_index, knot_list):
    knot_list_copy = knot_list.copy()
    knot_list_length = len(knot_list_copy)
    index = start_index
    # Reverse
    if index + command - 1 < knot_list_length:
        knot_list_copy[start_index:index + command] = knot_list_copy[index:index + command][::-1]
    else:
        start_pos = index
        end_pos = (index + command) % knot_list_length

        knot_list_temp = knot_list_copy[start_pos:] + knot_list_copy[:end_pos]
        knot_list_temp = knot_list_temp[::-1]

        knot_list_copy[index:] = knot_list_temp[:len(knot_list_temp) - end_pos]
        knot_list_copy[:end_pos] = knot_list_temp[len(knot_list_temp) - end_pos:]

    return knot_list_copy


def commands_build(command_line):
    commands = []
    for character in command_line:
        commands.append(ord(character))
