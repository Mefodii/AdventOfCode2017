...
# [ ]             [ ]     [ ]
#                 [ ]     [ ]
# Within each layer, a security scanner moves back and forth within its range. Each security scanner starts at the
# top and moves down until it reaches the bottom, then moves up until it reaches the top, and repeats. A security
# scanner takes one picosecond to move one step. Drawing scanners as S, the first few picoseconds look like this:
#
#
# Picosecond 0:
#  0   1   2   3   4   5   6
...
def build_layer(layer_data):
    processed_layer_data = [int(i) for i in layer_data.split(": ")]

    # return {processed_layer_data[0]: processed_layer_data[1]}
    return [int(i) for i in layer_data.split(": ")]


def calculate_trip_severity(layer_list):
    trip_severity = 0

    for layer in layer_list:
        scanner_layer = layer[0]
        scanner_range = layer[1]

        scanner_position = calculate_scanner_position(scanner_layer, scanner_range)
        if scanner_position == 0:
            trip_severity += scanner_layer * scanner_range

    return trip_severity


def calculate_scanner_position(scanner_layer, scanner_range):
    scanner_range_relative = scanner_range * 2 - 2
    scanner_position_relative = scanner_layer % scanner_range_relative

    if scanner_position_relative // scanner_range:
        return scanner_range_relative - scanner_position_relative
    else:
        return scanner_position_relative


def safe_trip(layer_list):
    delay = 0
    run = True
    next_delay = False
    ...
    while run:
        for layer in layer_list:
            ...
            scanner_position = calculate_scanner_position(scanner_layer + delay, scanner_range)
            if scanner_position == 0:
                next_delay = True
                break
...
def __main__(input_file):
    ...
    for input_line in input_file:
        layer_list.append(build_layer(input_line.replace("\n","")))

    safe_trip_delay = safe_trip(layer_list)

    print("##--RESULT--##")
    print("Delay for the safe trip:", safe_trip_delay)
