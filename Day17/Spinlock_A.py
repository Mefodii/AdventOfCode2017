...
# This spinlock isn't just consuming computing power, but memory, too; vast, digital mountains are being ripped from the
...
def spinlock(steps_forward_string):
    ...
    while spinlock_current_value < 10:
        spinlock_current_value += 1
        spinlock_next_index = (spinlock_index + steps_forward_int) % len(spinlock_memory)
        spinlock_memory.insert(spinlock_next_index, spinlock_current_value)

        spinlock_index = spinlock_next_index

    print(spinlock_memory)
...
def __main__(input_file):
    ...
    result = spinlock(input_file.read())
...
if __name__ == "__main__":
    ...
    # Input file
    input_file = open('../Input/Day17-Test.txt', 'r')
