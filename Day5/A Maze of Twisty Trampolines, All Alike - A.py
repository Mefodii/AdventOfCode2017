...
# Prepare input
start = time.time()
f = open('../Input/Day5-A.txt', 'r')

# Process
jumps = []
for jump_value in f:
    jumps.append(int(jump_value))

steps_taken = 0
current_index = 0
next_index = 0
while len(jumps) > next_index:
    current_index = next_index
    jump_distance = jumps[current_index]

    next_index = current_index + jump_distance
    jumps[current_index] += 1
    steps_taken += 1

    # print("##--DEBUG--##")
    # print("Current position: " + str(current_index), "Next position: " + str(next_index))
    # print(jumps)
