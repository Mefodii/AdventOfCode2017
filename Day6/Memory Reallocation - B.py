...
# continues doing this until it runs out of blocks; if it reaches the last memory bank, it wraps around to the
# first one.
#
...
#       redistribution.
#   -   Starting with the next bank (the fourth bank) and then continuing to the first bank, the second bank, and so
#       on, the 7 blocks are spread out over the memory banks. The fourth, first, and second banks get two blocks each,
...
#       wins the tie, and its three blocks are distributed evenly over the other three banks, leaving it
#       with none: 0 2 3 4.
...
# Prepare input
start = time.time()
f = open('../Input/Day6-B.txt', 'r')

# Process
banks = [int(value) for value in f.read().split()]
banks_len = len(banks)
states_log_sorted = set()
states_log_not_sorted = []
states_log_len = 0
steps = 0
cycle_distance = 0
run = True
...
while run:
    # print(banks)
    current_log = ','.join(str(value) for value in banks)
    states_log_sorted.add(current_log)
    states_log_not_sorted.append(current_log)

    max_value = max(banks)
    max_value_index = banks.index(max_value)
    banks[max_value_index] = 0
    for i in range(banks_len):
        add_value = math.ceil((max_value - i) / banks_len)
        index = (max_value_index + i + 1) % banks_len
        banks[index] += add_value
        # print(index, "Added: " + str(add_value))

    if len(states_log_sorted) > states_log_len:
        states_log_len += 1
        steps += 1
    else:
        cycle_distance = steps - states_log_not_sorted.index(current_log)
        run = False


print("##--RESULT--##")
print("Cycle distance: " + str(cycle_distance))
